  ...
  return (
    <div className="p-4 space-y-2 overflow-y-auto max-h-[calc(100vh-200px)]">
      {stations.map((station) => {
        // Enrich station to get computed fields
        ...
        return (
          <div
            ...
            style={{
              borderLeft: enriched.colors
                ? `3px solid ${enriched.colors.dominant}`
                : undefined,
            }}
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  background: enriched.colors
                    ? `linear-gradient(135deg, ${enriched.colors.dominant}20, ${enriched.colors.secondary}20)`
                    : undefined,
                }}
              >
              ...
              <div className="flex-1 min-w-0 text-left">
                <div className="flex items-center gap-2">
                  ...
                  <QualityBadge tier={enriched.qualityTier} />
                  <PopularityIndicator
                    score={enriched.popularityScore}
                    tier={enriched.popularityTier}
                  />
                </div>

                {enriched.displayLocation && (
                  <div className="flex items-center gap-1 mt-0.5 text-[10px] text-muted-foreground">
                    <MapPin className="w-3 h-3" />
                    <span className="truncate">{enriched.displayLocation}</span>
                  </div>
                )}

                <div className="mt-1.5">
                  <GenrePills
                    genres={enriched.subGenres}
                    primaryGenre={enriched.primaryGenre}
                    max={2}
